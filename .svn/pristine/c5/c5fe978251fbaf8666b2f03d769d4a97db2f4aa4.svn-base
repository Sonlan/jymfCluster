	  ...
	  if(null!=mailInfo.getAttachFileNames() && mailInfo.getAttachFileNames().length>0){
		  for(String filePath : mailInfo.getAttachFileNames()){
			  ...
			  if(file.exists()){
				 ...
				 attach.setDataHandler(new DataHandler(fds));//存入附件
				 attach.setFileName(fds.getName());//存入附件名
	  ...
	  if(null!=mailInfo.getAttachFileNames() && mailInfo.getAttachFileNames().length>0){
		  for(String filePath : mailInfo.getAttachFileNames()){
			  ...
			  if(file.exists()){
				 ...
				 html.setDataHandler(new DataHandler(fds));//存入附件
				 html.setFileName(fds.getName());//存入附件名
				 mainPart.addBodyPart(html);
			  }
		  }
	  }
