...
<script>
    $().ready(function() {
        ...
        $("#backBtn").click(function() {
           /*  $(location).attr('href', 'javascript:history.go(-1)'); */
            $(location).attr('href', '${ctx}/monitor/company/salesCntQuery/${productback}');
        });
    });
...
<body>
<form:form id="fenye" name="fenye" action="${ctx}/monitor/company/product/queryopt" method="post">
    ...
    <table id="table_1" class="jymf_table table_border">
        ...
        <tr class="jymf_tr table_1_tr">
            ...
            <td> <fmt:formatDate value="${optAction.optTime}" type="both" pattern="yyyy/MM/dd"/></td>
            <td>
