	...
	<form:form id="fenye" name="fenye" modelAttribute="company"
		...
		<div id="content_top">
			<table class="jymf_table">
				<tr class="table_1_tr">
					<td>企业名称&nbsp; <input name="name" type="text" class="texts"
						 />
					</td>
					<td>企业ID&nbsp;&nbsp; <input name="id" type="text"
						class="texts"/>
					</td>
 			...
 			<c:if test="${index eq '0' || index eq '2'}">
				...
				<c:forEach var="monitor" items="${pageView.records}">
					<tr class="jymf_tr table_1_tr">
						...
						<td align="center">
							<div class="small_btn">
								<c:if test="${monitor.proType ne '3' }">
									<a class="linke"
										href="${ctx}/monitor/moreMonitor/${monitor.areaCode}/${monitor.proType}"
										id="editLink-${monitor.id}">查看</a>
								</c:if>
								<c:if test="${monitor.proType eq '3' }">
									<a class="linke"
